...
def check_admin():
    if not st.session_state.admin_logged_in:
        st.sidebar.markdown("### 🔐 Area Admin")
...
gc = get_gc()
if gc:
    df_giocatori, ws_giocatori = get_sheet_data(gc, SHEET_NAME, "Giocatori")
    df_tornei, ws_tornei = get_sheet_data(gc, SHEET_NAME, "Tornei")
    df_partite, ws_partite = get_sheet_data(gc, SHEET_NAME, "Partite")
else:
    st.stop()
...
if menu == "Home":
    ...
    with col1: st.metric("Giocatori", len(df_giocatori))
    with col2: st.metric("Tornei", len(df_tornei))
    with col3: st.metric("Partite Giocate", len(df_partite))
    
    st.markdown("### 📜 Regolamento")
    st.info("""
    - **Sistema di Rating:** Elo (K=32)
...
# --- CLASSIFICA ---
    ...
    if not df_giocatori.empty:
        ...
        if search:
            df_filtered = df_giocatori[df_giocatori['Nome'].str.contains(search, case=False)]
        else:
    ...
    if not df_tornei.empty:
        ...
        if not tornei_aperti.empty:
            ...
            if rating_auto:
                ...
                rating_display = 1500
                st.info("Rating iniziale: **1500**")
            
            if st.button("📤 Invia Iscrizione"):
                st.success("""
                ✅ **Iscrizione registrata!**
                
                ⚠️ **Nota:** In questa versione, l'iscrizione va confermata manualmente dall'organizzatore.
                Contatta l'admin per completare la registrazione.
                """)
                st.info(f"Dettagli da inviare all'admin:\n- Torneo: {torneo_sel}\n- Nome: {nome_giocatore}\n- Email: {email}\n- Rating: {rating_display}")
        else:
            st.info("🔔 Non ci sono tornei aperti al momento. Torna più tardi!")
    else:
...
# --- LOGIN ADMIN ---
    ...
    if st.button("Accedi"):
        if password == st.secrets["admin"]["password"]:
            st.session_state.admin_logged_in = True
            st.success("✅ Accesso effettuato! Ricarica la pagina.")
            st.rerun()
        else:
...
# --- ADMIN PANEL ---
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📝 Crea Torneo", "🎮 Inserisci Risultati", "👤 Gestisci Giocatori", "⚙️ Impostazioni"])
    ...
    with tab2:
        ...
        if not df_tornei.empty:
            ...
            if st.button("🔄 Genera Abbinamenti"):
                players = df_giocatori.to_dict('records')
            ...
            if 'pairings' in st.session_state:
                ...
                round_num = len(df_partite[df_partite['ID_Torneo']==id_torneo]['Round'].unique()) + 1
    ...
    with tab3:
        ...
        with st.expander("➕ Aggiungi Giocatore"):
            ...
            rating = st.number_input("Rating Iniziale", value=1500)
    ...
    with tab4:
        ...
        st.write(f"**Stato Login:** {'✅ Admin' if st.session_state.admin_logged_in else '❌ Ospite'}")
        st.write(f"**Giocatori Totali:** {len(df_giocatori)}")
        st.write(f"**Partite Totali:** {len(df_partite)}")
