...
def fetch_sheet_data(_spreadsheet, worksheet_name):
    """Legge un singolo foglio - CACHED (underscore ignora l'hash)"""
    try:
...
def get_all_data_cached(gc, sheet_name, cache_key):
    """Legge tutti i fogli con caching"""
    try:
...
def swiss_pairing(players, past_matches):
    ...
    for i, p1 in enumerate(players_sorted):
        if p1['Nome'] in paired: continue
        for p2 in players_sorted[i+1:]:
            if p2['Nome'] in paired: continue
            history = [(m['Giocatore1'], m['Giocatore2']) for m in past_matches if m.get('Giocatore2')]
            if (p1['Nome'], p2['Nome']) in history or (p2['Nome'], p1['Nome']) in history: continue
...
if 'data_cache_key' not in st.session_state: st.session_state.data_cache_key = 0

def check_admin():
    if not st.session_state.admin_logged_in: st.warning("🔒 Accesso admin richiesto."); return False
    return True

def logout(): st.session_state.admin_logged_in = False; st.rerun()
def refresh_data(): st.session_state.data_cache_key += 1; st.rerun()

# --- INTERFACCIA ---
st.title("♟️ FFchess")

if st.session_state.admin_logged_in:
    if st.sidebar.button("🔄 Refresh Dati"): refresh_data()
...
menu_opts += ["🛡️ Admin", "🚪 Logout"] if st.session_state.admin_logged_in else ["🔐 Login Admin"]
menu = st.sidebar.selectbox("Menu", menu_opts)
...
if menu == "🏠 Home":
    ...
    with c1: st.metric("Giocatori", len(df_giocatori) if not df_giocatori.empty else 0)
    with c2: st.metric("Tornei", len(df_tornei) if not df_tornei.empty else 0)
    with c3: st.metric("Partite", len(df_partite) if not df_partite.empty else 0)
    st.divider()
    if not df_tornei.empty: st.dataframe(df_tornei.tail(5), use_container_width=True)

# --- CLASSIFICA FIDE ---
elif menu == "🏆 Classifica FIDE":
    st.header("🏆 Classifica Generale FIDE")
    if not df_giocatori.empty:
        s = st.text_input("🔍 Cerca")
        df_s = df_giocatori[df_giocatori['Nome'].str.contains(s, case=False, na=False)] if s else df_giocatori
...
# --- TORNEI ---
    ...
    if not df_tornei.empty:
        sel = st.selectbox("Seleziona", df_tornei['Nome'].tolist())
        t = df_tornei[df_tornei['Nome']==sel].iloc[0]; tid = t['ID_Torneo']
        c1,c2,c3 = st.columns(3)
        with c1: st.metric("Formato", t['Tipo'])
        with c2: st.metric("Stato", t['Stato'])
        with c3: st.metric("Data", t['Data'])
        st.divider()
        ...
        if not df_partite.empty and not df_partecipanti.empty:
            ...
            for n in part:
                ...
                for _,r in df_partite[(df_partite['ID_Torneo']==tid)&((df_partite['Giocatore1']==n)|(df_partite['Giocatore2']==n))].iterrows():
                    if pd.isna(r['Giocatore2']): p+=1.0
                    elif r['Giocatore1']==n:
                        if r['Risultato']=="1-0": p+=1
                        elif r['Risultato']=="0.5-0.5": p+=0.5
                    else:
                        if r['Risultato']=="0-1": p+=1
                        elif r['Risultato']=="0.5-0.5": p+=0.5
                pts[n]=p
            df_cl = pd.DataFrame([{"Nome":n,"Punti":pts,"FIDE":df_giocatori[df_giocatori['Nome']==n]['Rating'].values[0] if not df_giocatori[df_giocatori['Nome']==n].empty else 0} for n,pts in pts.items()]).sort_values("Punti",ascending=False)
            st.dataframe(df_cl.reset_index(drop=True), use_container_width=True)
        st.divider()
        if not df_partite.empty: st.dataframe(df_partite[df_partite['ID_Torneo']==tid], use_container_width=True)

# --- LOGIN ---
elif menu == "🔐 Login Admin":
    st.header("🔐 Accesso"); pwd = st.text_input("Password", type="password")
    if st.button("Accedi"):
        if pwd == st.secrets["admin"]["password"]: st.session_state.admin_logged_in = True; st.success("✅"); st.rerun()
        else: st.error("❌")
...
# --- ADMIN ---
    ...
    with tab1:
        ...
        c1,c2 = st.columns(2)
        with c1: nome=st.text_input("Nome"); tipo=st.selectbox("Formato",["Svizzero","Girone"])
        with c2: data=st.date_input("Data",datetime.now())
        bye=st.selectbox("Punti Bye",[1.0,0.5])
        part = st.multiselect("Giocatori", df_giocatori['Nome'].tolist()) if not df_giocatori.empty else []
        if st.button("📌 Crea", type="primary"):
            if nome and part:
                def _cr():
                    tid=f"TOR_{datetime.now().strftime('%Y%m%d%H%M%S')}"; ws_tornei.append_row([tid,nome,str(data),tipo,"In Corso",bye])
                    if ws_partecipanti:
                        for p in part: ws_partecipanti.append_row([tid,p])
                    return tid
                r=safe_api_call(_cr)
                if r: st.success(f"✅ Creato! {len(part)} giocatori"); refresh_data()
    
    with tab2:
        st.subheader("🎮 Gestisci Partite")
        if not df_tornei.empty:
            sel=st.selectbox("Torneo",df_tornei['Nome'].tolist(),key="at"); t=df_tornei[df_tornei['Nome']==sel].iloc[0]; tid,bye_val=t['ID_Torneo'],t.get('Bye',1.0)
            pn=df_partecipanti[df_partecipanti['ID_Torneo']==tid]['Nome'].tolist() if not df_partecipanti.empty else []
            pl=df_giocatori[df_giocatori['Nome'].isin(pn)].to_dict('records') if df_giocatori is not None else []
            if st.button("🔄 Genera Round"):
                past=df_partite[df_partite['ID_Torneo']==tid].to_dict('records') if not df_partite.empty else []
                pr,bye=swiss_pairing(pl,past); st.session_state.update({'pairings':pr,'bye':bye,'round':df_partite[df_partite['ID_Torneo']==tid]['Round'].max()+1 if not df_partite.empty else 1})
                ...
                if st.button("💾 Salva",type="primary"):
                    def _sv():
                        for i,(p1,p2) in enumerate(st.session_state['pairings']):
                            ...
                            if p2 is None: ws_partite.append_row([tid,rn,p1['Nome'],None,"Bye"])
                            else:
                                ...
                                if ws_giocatori:
                                    for nm,v in [(p1['Nome'],n1),(p2['Nome'],n2)]:
                                        idx=df_giocatori[df_giocatori['Nome']==nm].index[0]+2; ws_giocatori.update_cell(idx,3,v)
                    safe_api_call(_sv); st.success("✅ Salvato!"); st.session_state.pop('pairings',None); refresh_data()
    ...
    with tab3:
        ...
        with st.expander("➕ Aggiungi"):
            nm,rt=st.text_input("Nome"),st.number_input("Rating",value=RATING_INIZIALE)
            if st.button("Aggiungi") and ws_giocatori:
                safe_api_call(lambda: ws_giocatori.append_row([f"PL_{datetime.now().strftime('%Y%m%d')}",nm,rt,""])); st.success("✅"); refresh_data()
